...
async def test_get_user_query_reads_through_a_read_handler() -> None:
    ...
    registered = await handler.execute(
...
async def test_activate_user_command_persists_the_aggregate_and_domain_event() -> None:
    repository = FakeUserRepository()
    register_handler = RegisterUserCommandHandler(
        FakeIdentityUnitOfWork(repository), FakePasswordHasher()
    )
    await register_handler.execute(RegisterUserCommand("user@example.com", "password1"))
    user = repository.users["user@example.com"]
    user.pull_events()
    user.is_active = False

    handler = ActivateUserCommandHandler(FakeIdentityUnitOfWork(repository))
    result = await handler.execute(ActivateUserCommand(target_user_id=user.id))
