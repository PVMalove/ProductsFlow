...
def test_register_persists_the_user_and_returns_ok() -> None:
    ...
    result = handler.execute(RegisterUserCommand("user@example.com", "password1"))
...
def test_register_fails_with_conflict_when_email_already_exists() -> None:
    ...
    first = handler.execute(RegisterUserCommand("user@example.com", "password1"))
    assert first.is_ok

    second = handler.execute(RegisterUserCommand("user@example.com", "password2"))
...
def test_register_does_not_construct_the_aggregate_on_a_duplicate_email() -> None:
    repository = FakeUserRepository()
    handler = RegisterUserCommandHandler(repository, FakePasswordHasher())
    handler.execute(RegisterUserCommand("user@example.com", "password1"))
    users_before = dict(repository.users)

    handler.execute(RegisterUserCommand("user@example.com", "password2"))
...
def test_register_fails_with_validation_on_a_weak_password() -> None:
    ...
    result = handler.execute(RegisterUserCommand("user@example.com", "short"))
...
def test_register_does_not_hash_a_weak_password() -> None:
    ...
    handler.execute(RegisterUserCommand("user@example.com", "short"))
