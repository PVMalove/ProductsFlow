...
async def test_worker_syncs_user_events_idempotently_and_by_version(
    ...
    try:
        for message_id, event_type, role, is_active in (
            (101, "user.registered.v1", None, None),
            (102, "user.activated.v1", None, None),
            (103, "user.deactivated.v1", None, None),
            (104, "user.role_changed.v1", "admin", None),
        ):
        ...
        assert stored_owner.is_active is False
