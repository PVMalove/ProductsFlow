...
from kernel_platform.http.match import match_created, match_result, unwrap_result
...
async def create_ticket(
    ...
    command: CreateTicketCommand = request.to_command(actor=actor)
...
async def list_tickets(
    ...
    query: ListTicketsQuery = request.to_query(actor=actor)
    page: TicketPage = await handler.execute(query)
    return ApiResponse(
...
async def list_admin_tickets(
    ...
    query: ListAdminTicketsQuery = request.to_query(actor=actor)
    page: TicketPage = unwrap_result(await handler.execute(query))
    return ApiResponse(
        data=[TicketView.from_domain(ticket) for ticket in page.items],
        meta=_page_meta(page.page_info),
    )
...
async def get_admin_ticket_detail(
    ...
    query: GetTicketDetailQuery = request.to_query(actor=actor)
    detail: TicketDetail = unwrap_result(await handler.execute(query))
...
async def add_ticket_message(
    ...
    command: AddTicketMessageCommand = request.to_command(
...
async def change_ticket_status(
    ...
    command: ChangeTicketStatusCommand = request.to_command(
        ticket_id=ticket_id, actor=actor
    )
    result: Result[TicketView] = await handler.execute(command)
...
async def edit_ticket_message(
    ...
    request: TicketMessageEditRequest,
    actor: RequiredActor,
    handler: EditTicketMessageDI,
) -> ApiResponse[TicketView]:
    command: EditTicketMessageCommand = request.to_command(
...
async def delete_ticket_message(
    ...
    command: DeleteTicketMessageCommand = request.to_command(actor=actor)
