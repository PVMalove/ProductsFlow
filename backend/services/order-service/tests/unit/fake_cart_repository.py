...
class FakeCartRepository:
    ...
    async def get_locked_for_user(self, user_id: uuid.UUID) -> Cart | None:
        # Фейку не нужен реальный SELECT ... FOR UPDATE — семантика чтения
        # идентична get_for_user (issue #372).
        return self._by_user_id.get(user_id)

    async def get_locked_by_order(self, order_id: uuid.UUID) -> Cart | None:
        for cart in self._by_user_id.values():
            if any(line.locked_by_order_id == order_id for line in cart.lines):
                return cart
        return None
