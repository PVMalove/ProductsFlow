...
class FakeIdentityClient:
    """Фейковый identity-клиент (ADR 0013, Seam A): не бьёт по сети,
    HTTP-слой inventory-service тестируется против настоящего Postgres, но
    против подставного identity. `register()` заводит токен, `unavailable`
    имитирует недоступность identity для проверки fail-closed (issue #367)."""
