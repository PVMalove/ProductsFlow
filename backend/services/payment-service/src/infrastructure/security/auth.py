...
class AuthContext:
    ...
    верификация, зеркалит catalog/inventory's `AuthContext`, ADR 0011). Любой
    аутентифицированный пользователь допускается ко всем 4 операциям
    (архитектурный бриф D6) — роль не проверяется, синхронный
    `fetch_current_user()` не требуется вовсе."""
