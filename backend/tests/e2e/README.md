...
В пайплайне на данный момент реализованы 4 глобальных сценария:
...
### 4. Жизненный цикл обращения в поддержку
**Файл:** `test_support_ticket_lifecycle.py`

Проверяет публичный API `support-service` через Gateway, не обращаясь к БД
или сервису напрямую.
- Посторонний пользователь не может прочитать или дополнить чужой тикет.
- Автор может исправить собственное сообщение; администратор отвечает и ведёт
  тикет через `OPEN → IN_PROGRESS → RESOLVED`.
- Новое сообщение автора возвращает тикет из `RESOLVED` в `IN_PROGRESS`; после
  `CLOSED` обычные изменения отклоняются с `409`.
- Администратор может модерировать сообщение даже в закрытом тикете, а API
  сохраняет его как мягко удалённое.
