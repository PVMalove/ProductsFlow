...
**Оркестрация sub-agent'ов (MUST):** если сессия работает под Orca (`TERM_PROGRAM=Orca` / `ORCA_WORKSPACE_ID` в окружении), любой сабагент с отдельным deliverable (тело PR, код, отчёт) **обязан** запускаться через Orca Run → Task → Dispatch, а координатор ждёт `worker_done` через `orca orchestration check --wait`, обрабатывает доставленные результаты и продолжает workflow автоматически. Прямой generic agent spawn обходит lifecycle Orca и **не используется** для deliverable-задач (допустим только для тривиального in-session lookup, например `Explore`). Нарушение этого правила — тот самый антипаттерн: координатор не видит завершения сабагента, workflow не продолжается сам, и пользователю приходится вручную инициировать каждый следующий шаг. В обычном рантайме (без Orca) действует fallback с уведомлениями harness.
