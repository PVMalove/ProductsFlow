...
- Prometheus собирает HTTP-метрики API, длительности и ошибки SQL-запросов, состояние SQLAlchemy connection pool и счётчики неожиданных исключений. Он также scrape-ит внутренний `/metrics` `catalog-search-worker` для lag/DLQ. Локальный overlay загружает rules для service health, 5xx, restart-loop, DB saturation/failures и Catalog Search; Alertmanager не входит в состав overlay, потому что адресаты и секреты доставки принадлежат конкретной среде. `/metrics` не проходит через Gateway и исключён из HTTP-метрик и трейсов; метки не содержат пользовательских данных или значений с высокой кардинальностью.
...
- Для dev-профиля стек запускается через `make up_monitoring`: target объединяет base, `docker-compose.dev.yml` и monitoring overlay, сохраняя публикации Gateway/API. Для prod запускайте явную Compose-команду с `docker-compose.prod.yml` вместо dev override.
