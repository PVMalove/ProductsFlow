...
- Rollout атомарен по зависимостям: shared Actor, identity event contracts и support schema идут до worker/projection/authentication, а support thin endpoints — только после их готовности. Catalog по умолчанию получает только compatibility-refactor импортов Actor; его неохваченные BFF endpoint'ы не входят в эту миграцию. Исключение — `product_images.py`: его перевели на тот же конверт и `200 data: null` для DELETE отдельным решением в рамках этого PR, не меняя command/query-разделение catalog (`ProductImageMutation` по-прежнему не несёт query-View — `GET`-хендлер после `upsert` остаётся отдельным вызовом, это не тот orchestration-анти-паттерн, который другие правила этого ADR запрещают).
