...
Целевая платформа состоит из шести независимо разворачиваемых микросервисов и общего, не разворачиваемого самостоятельно Shared Kernel:
...
| `order-service`    | Владеет серверной корзиной (`Cart`), заказом (`Order`) и оркестрацией checkout-Saga                                                                                                                                     |
| `inventory-service`| Владеет доступным остатком, резервом и allocation товара                                                                                                                                                                |
| `payment-service`  | Владеет авторизацией, capture и интеграцией с Test PSP                                                                                                                                                                   |
...
| `test-support`     | Dev-only testcontainers-фикстуры, общие для интеграционных тестов сервисов                                                                                                                                              |

Каждый сервис — собственный процесс, собственная база данных (`identity-db`/`catalog-db`/`support-db`/`order-db`/`inventory-db`/`payment-db`), собственный Alembic, собственный Docker-образ. Данные между сервисами не расшариваются напрямую ни через общую БД, ни через прямой SQL. RabbitMQ — стандартный канал адресных команд и доменных событий; узкие синхронные границы `order-service` → Catalog для checkout-quote и `order-service` → Identity для проверки активного пользователя явно разрешены [ADR 0016](0016-ordering-saga-and-bounded-contexts.md).
...
- **Ordering** — серверная корзина, заказ, коммерческий снимок и устойчивый автомат orchestration-Saga; не владеет остатком, каталогом или платежной операцией ([ADR 0016](0016-ordering-saga-and-bounded-contexts.md)).
- **Inventory** — доступный остаток, reservation и allocation; не владеет товарной карточкой или заказом ([ADR 0016](0016-ordering-saga-and-bounded-contexts.md)).
- **Payment** — авторизация, capture и reconciliation платежа; не владеет заказом или остатком ([ADR 0016](0016-ordering-saga-and-bounded-contexts.md)).
