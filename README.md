...
## Что реализовано (Завершение Фаз 0–6)

- **Строгий CQRS & Тонкие Эндпоинты**: Во всех микросервисах завершен рефакторинг API. Роутеры FastAPI больше не содержат бизнес-логики. Маппинг данных полностью инкапсулирован в Request-моделях, а ответы унифицированы в общую BFF-оболочку (`data`, `meta`, `error`) с помощью библиотек платформы.
- **Безопасные идентификаторы (UUIDv4)**: Полный отказ от предсказуемых целочисленных ключей во всей системе. Это предотвращает IDOR-уязвимости и скрывает бизнес-метрики (количество товаров, пользователей).
- **Безопасный Startup и Инфраструктура**: Инициализация БД, Alembic-миграции и сидирование (bootstrap) полностью вырезаны из `lifespan` хуков FastAPI. Теперь они выполняются в строгом порядке через `init-контейнеры` (команда `make setup` / `make demo`).
- **Изолированное тестирование**: Монолитные тесты уничтожены. Для предотвращения связанности каждый микросервис теперь содержит только свои собственные юнит- и интеграционные тесты в локальной папке `tests/`. Покрытие превышает 300 тестов, которые успешно проходят строгие архитектурные проверки (`check_architecture.py`).
- **Shared Kernel & EDA**: Интеграция общей библиотеки `kernel-domain` (чистые интерфейсы) и `kernel-platform` (инфраструктура RabbitMQ, JWT, Transactional Outbox). Межсервисная асинхронная хореография полностью настроена.
...
Внешнее взаимодействие с сервисами осуществляется через API Gateway, который маршрутизирует запросы к соответствующим микросервисам. Все идентификаторы сущностей в системе используют **UUIDv4**.

### Identity Service
- **Управление аккаунтами**: Регистрация, удаление профиля (генерирует событие `user.deleted` в Outbox).
- **Аутентификация**: Выдача Stateless JWT access-токенов (подписанных RS256).
- **Профили**: Получение данных профиля, смена пароля.
- **Администрирование**: Управление списком пользователей (доступно только для `admin`).

### Catalog Service
- **Каталог и поиск**: Получение списка товаров (курсорная пагинация) и поиск с учетом Read Models (скрывающих деактивированные товары).
- **Управление товарами**: Создание (CQRS Command, генерирует доменное событие), частичное редактирование, де/активация.
- **Медиа**: Загрузка и привязка изображений (сохраняются в MinIO).
- **Удаление**: Безвозвратное удаление записей о товарах (с сохранением audit-лога).
- **Доступ**: Все мутирующие операции доступны только владельцу товара или администратору.

### Support Service
- **Обращения**: Создание тикетов в службу поддержки.
- **История**: Просмотр списка своих тикетов и их статуса.
- **Приватность**: При получении события удаления аккаунта (`user.deleted`) история обращений автоматически анонимизируется.
